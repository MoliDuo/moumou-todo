...
- 已完成视图：点输入框右边的日历按钮切换，已完成的任务按打勾那天分组（今天 / 昨天 / 前天，更早的显示「9月20日 星期日」），每组显示当天总耗时；点任务右边的时长可填写耗时（`45`、`45分钟`、`1h30m`、`1小时30分`、`1:30`、`1.5h` 都行，不填算 0）
...
app/
  ...
  renderer/                浮窗页面（HTML/CSS/JS，还原设计稿样式）
    ...
    history.js             已完成视图的按天分组、日期标签、耗时解析/格式化
